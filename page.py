...
# Skip string
DESCRIBE_SALES = 'Bạn đủ điều kiện nhận giảm giá'
DESCRIBE_LOGIN = 'Để tiết kiệm tại chỗ nghỉ này, bạn chỉ cầnđăng nhập'


def start_thread():
    dalat = load_file_json('da_lat_link.json')
    danang = load_file_json('da_nang_link.json')
    nhatrang = load_file_json('nha_trang_link.json')
    phuquoc = load_file_json('phu_quoc_link.json')
    vungtau = load_file_json('vung_tau_link.json')
    hoian = load_file_json('hoi_an_link.json')
    ...
    thread6 = Thread(target=crawl, args=(phuquoc, 6, '6_hoian',))
    # Start Thread
    thread1.start()
    thread2.start()
    thread3.start()
...
def get_content_page(link, idx, idLocation, chrome):
    ...
    print("Sleep 30s")
    sleep(30)
    print(dom.xpath('//*[@id="basiclayout"]/div[1]/div/div[21]/div/div/div/div/section/div/div[2]/div[2]/div[1]/ul/li[1]/div/span/div/div[1]'))
    q_span = soup.find('li', class_='ef20942686 f514bc8c68')
    print(q_span)
    # query
    ...
    try:
        ...
        description = q_description.get_text(strip=True).replace('Tôi muốn xem thêm', ''). \
            replace(DESCRIBE_LOGIN, ''). \
            replace(DESCRIBE_SALES, '')
        ...
        try:
            ...
            cost_new = q_cost_new[0].get_text(strip=True).replace('VND', '').replace('\xa0', ' ')
        except:
            try:
                q_cost_original = soup.find_all('div',
                                                class_='bui-price-display__value prco-text-nowrap-helper prco-inline-block-maker-helper prco-f-font-heading')
                cost_original = cost_original = q_cost_original[0].get_text(strip=True).replace('VND', '').replace(
                    '\xa0', ' ')
...
def get_images(chrome, link):
    ...
    try:
        ...
        try:
            open_pop_images = chrome.find_element(By.CLASS_NAME,
                                                  'bh-photo-grid-item.bh-photo-grid-thumb.js-bh-photo-grid-item-see-all')
...
def test_get_page():
    link = 'https://www.booking.com/hotel/vn/reddoorz-near-thung-lung-tinh-yeu.vi.html?aid=304142&label=gen173nr-1FCAEoggI46AdIM1gEaPQBiAEBmAEquAEZyAEM2AEB6AEB-AELiAIBqAIDuALEh6WbBsACAdICJGFhMDI0NDI3LTczZGMtNDQ4Yi04Mjc4LTBjNzZhMmQxMGRiNdgCBuACAQ&sid=b915d352fe7e9ae8eb5d632d2e55a1cf&all_sr_blocks=563346001_358151778_0_0_0;checkin=2022-12-01;checkout=2022-12-02;dest_id=-3712045;dest_type=city;dist=0;group_adults=2;group_children=0;hapos=3;highlighted_blocks=563346001_358151778_0_0_0;hpos=3;matching_block_id=563346001_358151778_0_0_0;no_rooms=1;req_adults=2;req_children=0;room1=A%2CA;sb_price_type=total;sr_order=popularity;sr_pri_blocks=563346001_358151778_0_0_0__31118200;srepoch=1667920123;srpvid=1a216a7db8b70182;type=total;ucfs=1&#hotelTmpl'
    ...
    category = 1
    chrome = webdriver.Chrome(service=Service(ChromeDriverManager().install()))
    export_data_to_json('test', get_content_page(link, idx, category, chrome))
